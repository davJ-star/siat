...
endpoint랑 url이랑 차이가 있다. 

--> url에서 props를 통해서 elements를 통해서 바꿔진다.(컴포넌트 변경등..)
이걸 broswerRouter를 통해 체크~!
...
=> (state) props에 해당 될 것이다.


=> router-dom: 화면이동을 진행한다.
...
state로 화면상태를 감지한다.
필요에 따라 페이지를 바꿔주는 작업이 필요하다.


// 랜더링될때 endpoint에 대해 요청을 던져준다.
useEffect(, [])
